...
import { saveCitiesSettingsAction } from "@/app/settings/cities/actions";
...
function SubmitButton() {
  ...
  return (
    ...
    >
      {pending ? "Saving…" : "Save & refresh targets →"}
    </button>
  );
}

export function SettingsCitiesForm({
  defaultCities,
  defaultRadius,
  defaultCategories,
  error,
  ...
  const [cities, setCities] = useState<CityRow[]>(
    defaultCities.length > 0 ? defaultCities : [{ city: "", state: "" }]
  );
  ...
  return (
    <form action={saveCitiesSettingsAction} className="auth-form">
      {error ? (
        <div
          role="alert"
          style={{
      ...
      >
        Changing cities or categories kicks off a fresh Google search for any
        new combos. Existing businesses on your list stay put.
      </p>
